...
def scan_header(framework, header_fn):
    ...
    with codecs.open(header_fn, 'r', encoding='utf-8',
                     errors='ignore') as fd:
        lines = fd.readlines()
...
if __name__ == '__main__':
    ...
    defaultRoot = sh.xcrun("--sdk", "iphoneos", "--show-sdk-path").strip()
    sysroot = os.getenv("SYSROOT", defaultRoot)
    search_frameworks(os.path.join(sysroot, 'System/Library/Frameworks'))
    fn = join(dirname(__file__), '..', 'pyobjus', 'protocols.py')
    _info = "autogenerated by buildprotocols.py for {}".format(sysroot)
